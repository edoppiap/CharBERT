...
class IrEvaluator:
    def __init__(self,
                ...
                persist_db_path = '/content/drive/MyDrive/PoliTo/NLP_Polito/progetto/chromaDbs'
                ):
        ...
        self.persist_db_path = Path(persist_db_path) / self.prj_name
    ...
    def create_db(self, chunk_size = 2000, chunk_overlap = 200, load_db = False, persist_db = False):
        if load_db:
            self.db= Chroma(persist_directory = str(self.persist_db_path), embedding_function = self.embeddings)
            print('Loaded Db with', len(self.db.get()['ids']), 'chunks')
            ...
            if persist_db:
                self.db = Chroma.from_documents(texts, self.embeddings, persist_directory = str(self.persist_db_path))
            else:
    ...
    def evaluate_retriever_via_df(self, code_query = False, k = None, search_type = None):
