...
def main():
    ...
    parser.add_argument("--file_path", default=None, type=str, required=True,
                        help="Path to the .txt file to be processed.")
    ...
    file_path = args.file_path if args.file_path.endswith('.txt') else args.file_path + '.txt'
    ...
    with open(file_path, 'r') as file, open(output_dir/out_file_name, 'w') as write_file:
    ...
    print("Length of original dataset: ", sum(1 for _ in open(file_path)))
    print("Length of adversarial dataset: ", sum(1 for _ in open(output_dir/out_file_name)))
